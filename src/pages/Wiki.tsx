...
const Wiki: React.FC = () => {
	...
	const categories = pages ? Array.from(new Set(pages.map(p => p.category))) : []
			...
			<div className="flex flex-col gap-8">
				<header className="flex flex-col gap-4">
					...
					<p className="text-white/50 text-lg max-w-2xl">
						Explore guides, tutorials, and deep-dives into Stellar, Soroban, and how to make the most of LearnVault.
					</p>
				</header>
				...
				{isLoading ? (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{[1, 2, 3].map(i => (
							<div key={i} className="glass-card h-48 animate-pulse rounded-2xl border border-white/5" />
						))}
					</div>
				) : (
									...
									{pages?.filter(p => p.category === category).map(page => (
										...
										>
											<div className="flex items-center justify-between">
												...
												<Icon.ChevronRight size="sm" className="text-white/20 group-hover:text-brand-cyan transition-colors" />
											</div>
										</Link>
									))}
								</div>
