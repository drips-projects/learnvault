...
const CommentSection: React.FC<CommentSectionProps> = ({ proposalId, proposalAuthor }) => {
  ...
  const fetchComments = async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:4000/api/proposals/${proposalId}/comments`);
  ...
  const handlePostComment = async (parentId: number | null = null) => {
    ...
    // Mock JWT token (In a real app, get this from auth context)
  ...
  return (
    <div className="mt-16 border-t border-white/5 pt-16">
      <div className="flex items-center justify-between mb-8">
        <h3 className="text-2xl font-black tracking-tight">{t("comments.title", "Discussion")}</h3>
          ...
          {(["top", "new", "oldest"] as const).map((sort) => (
            <button
              ...
              className={`text-xs font-black uppercase tracking-widest px-4 py-2 rounded-full transition-all ${
                sortBy === sort ? "bg-brand-cyan text-black" : "bg-white/5 text-white/40 hover:bg-white/10"
              }`}
            >
              {sort}
            </button>
      ...
      <div className="mb-12">
        <textarea
          ...
          placeholder={t("comments.placeholder", "Share your thoughts... (Markdown supported)")}
