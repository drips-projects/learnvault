...
const CommentCard: React.FC<CommentCardProps> = ({ comment, isAuthor, isReply, canPin, onUpdate }) => {
  ...
  const handleVote = async (type: "upvote" | "downvote") => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/comments/${comment.id}/vote`, {
        method: "PUT",
        headers: {
  ...
  const handlePin = async () => {
    const token = localStorage.getItem("auth_token") || "mock-token";
    try {
      const res = await fetch(`http://localhost:4000/api/comments/${comment.id}/pin`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`
        }
  ...
  return (
    <div className={`glass-card p-6 rounded-3xl border border-white/5 relative ${comment.is_pinned ? "border-brand-cyan/30 bg-brand-cyan/5" : ""}`}>
      {comment.is_pinned && (
      ...
      <header className="flex justify-between items-start mb-6">
        ...
        <div className="flex gap-2">
          {canPin && !comment.is_pinned && (
            <button onClick={handlePin} className="text-[10px] font-black uppercase text-white/30 hover:text-brand-cyan transition-colors">
              Pin
            </button>
          )}
          {!isReply && (
      ...
      {isReplying && (
        <div className="mt-8 pt-8 border-t border-white/5 animate-in slide-in-from-top-4 duration-500">
          ...
          <div className="flex justify-end gap-3 mt-4">
            <button onClick={() => setIsReplying(false)} className="px-5 py-2 text-[10px] font-black uppercase text-white/30 border border-white/10 rounded-full hover:bg-white/5 transition-colors">
              Cancel
            </button>
            <button onClick={handlePostReply} disabled={!replyText.trim()} className="px-5 py-2 bg-brand-cyan text-black text-[10px] font-black uppercase tracking-widest rounded-full hover:scale-105 transition-all disabled:opacity-50">
              Submit Reply
            </button>
          </div>
        </div>
